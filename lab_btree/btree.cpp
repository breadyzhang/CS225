...
V BTree<K, V>::find(const BTreeNode* subroot, const K& key) const
{
    ...
    if(subroot->elements[idx].key == key){
...
void BTree<K, V>::insert(BTreeNode* subroot, const DataPair& pair)
{
    ...
    /* TODO Your code goes here! */
    if(subroot->is_leaf && idx == subroot->elements.size()){
      subroot->elements.insert(subroot->elements.begin() + idx, pair);
    }
    else if(subroot->is_leaf && subroot->elements[idx].key != pair.key){
      subroot->elements.insert(subroot->elements.begin() + idx, pair);
    }
    else if(!subroot->is_leaf && (idx == subroot->elements.size() || subroot->elements[idx].key != pair.key)){
